...
def Database():
    ...
    conn = mysql.connector.connect(
        ...
        password="Mysql",  # change to your password
        database="studentdb"  # change to your database
    )
    cursor = conn.cursor()
    #creating STUD_REGISTRATION table
    cursor.execute(
        "CREATE TABLE IF NOT EXISTS STUD_REGISTRATION (STU_ID INT AUTO_INCREMENT PRIMARY KEY, STU_NAME VARCHAR(255), STU_CONTACT VARCHAR(255), STU_EMAIL VARCHAR(255), STU_ROLLNO VARCHAR(255), STU_BRANCH VARCHAR(255))")
...
def register():
    ...
    if name1 == '' or con1 == '' or email1 == '' or rol1 == '' or branch1 == '':
        ...
        #execute query
        cursor.execute('INSERT INTO STUD_REGISTRATION (STU_NAME, STU_CONTACT, STU_EMAIL, STU_ROLLNO, STU_BRANCH) VALUES (%s, %s, %s, %s, %s)',
                       (name1, con1, email1, rol1, branch1))
        conn.commit()
...
def Delete():
    ...
    if not tree.selection():
        ...
        if result == 'yes':
            ...
            tree.delete(curItem)
            cursor.execute("DELETE FROM STUD_REGISTRATION WHERE STU_ID = %s", (selecteditem[0],))
            conn.commit()
            conn.close()
...
def SearchRecord():
    ...
    if SEARCH.get() != "":
        ...
        cursor.execute("SELECT * FROM STUD_REGISTRATION WHERE STU_NAME LIKE %s", ('%' + str(SEARCH.get()) + '%',))
        ...
        conn.close()
#defining function to access data from MySQL database
def DisplayData():
    #open database
    Database()
    #clear current data
